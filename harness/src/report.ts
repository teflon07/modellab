...
export function renderMarkdown(cells: CellSummary[], meta: ReportMeta): string {
  ...
  for (const track of TRACKS) {
    ...
    lines.push(`## ${track}`, "");
    lines.push("| spec | model | n | pass% | tokens (med) | cost (med) | cost-per-success | turns (med) | tps (med) | wall ms (med) |");
    lines.push("|---|---|---|---|---|---|---|---|---|---|");
    for (const c of group) {
      lines.push(
        `| ${c.specId} | ${c.model} | ${c.n} | ${(c.passRate * 100).toFixed(0)}% | ` +
        `${c.tokens.median} | ${c.cost.median.toFixed(4)} | ` +
        `${c.costPerSuccess === null ? "—" : c.costPerSuccess.toFixed(4)} | ` +
        `${c.turns.median} | ${c.outputTps.median} | ${c.wallClockMs.median} |`,
      );
    }
    lines.push("");
  }
...
export function renderCsv(cells: CellSummary[]): string {
  const header = [
    "track", "spec", "model", "n", "pass_rate",
    "tokens_med", "tokens_min", "tokens_max",
    "cost_med", "cost_per_success", "tokens_per_success",
