...
Three stacked levels; the path moves within a level (UDLR) and climbs/descends
ladders (+/-). PASS = the model's real solve. FAIL = an explicit move string, or
--auto-fail (beeline from S along the longest open run, never climbing, until it
clips a wall). Fail row on top, pass row below. On-brand dark, animated.
...
S_COL=(61,184,225); E_COL=(240,180,40); LAD=(240,205,90); OK=(61,184,225); BAD=(232,119,119)
CELL=20; HEAD=42; LGAP=26; ROWGAP=30; PAD=14
...
def auto_fail(L):
    ...
    for m in "RLDU":
        ...
        while True:
            ...
            if inb(L,zz,nr,nc) and L[zz][nr][nc]!="#": rr,cc=nr,nc; s+=m
            else: s+=m; break  # one step into the wall -> clip
...
def main():
    ...
    ap.add_argument("--fail-label", default="FAIL  ·  never climbs, hits a wall")
    ap.add_argument("--duration", type=int, default=130)
    a=ap.parse_args()
    D_=json.load(open(os.path.expanduser(a.data)))
    L=D_["levels"]; PASS=D_["path"]; nlev=len(L); side=len(L[0])
    FAIL=a.fail if a.fail else (auto_fail(L) if a.auto_fail else "")
    if not FAIL: ap.error("provide --fail MOVES or --auto-fail")
    plabel=a.pass_label or f"PASS  ·  {D_.get('model','model')}  ·  climbs to E"
    lw=side*CELL
    ...
    def draw_row(moves, reveal):
        ...
        for i in range(0,n+1):
            ...
            p=pt(z,r,c)
            if prev and prev[0]==z: d.line([prev[1],p], fill=(BAD if red else OK), width=4)
    ...
    CARDW=RW+2*PAD; CARDH=30+RH+ROWGAP+22+RH+PAD
    frames=[]
